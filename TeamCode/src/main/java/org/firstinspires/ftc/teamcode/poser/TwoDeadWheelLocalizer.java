...
public class TwoDeadWheelLocalizer implements DeltaLocalizer {
    ...
    public static final Distance X_ODO_LEVER_ARM = Distance.inInches(3).neg();
    public static final Distance Y_ODO_LEVER_ARM = Distance.inInches(7).neg();
    ...
    public TwoDeadWheelLocalizer(Hardware hardware) {
        this.odo = hardware.odo;
        this.imu = hardware.imu;

        this.xOdo = odo.rightEncoder();
        this.yOdo = odo.backEncoder();
    ...
    public Pose updateWithDelta() {
        int newXOdo = odo.rightEncoder();
        int newYOdo = odo.backEncoder();
